    ...
    <section className="pax-docs" style={{ marginTop: '3rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem' }}>
        {Object.entries(sellResults.passengers).map(([key, p]) => {
          ...
          return (
            <div key={key} className={`card ${status === 'success' ? 'success' : ''}`} style={{ borderTop: `6px solid ${status === 'success' ? '#27ae60' : 'var(--jetsmart-navy)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                  <div className="plane-icon-bg" style={{ background: status === 'success' ? '#27ae60' : 'var(--jetsmart-navy)', width: '35px', height: '35px' }}><User size={18} color="white" /></div>
                  <b style={{ color: 'var(--jetsmart-navy)', fontSize: '1.1rem' }}>{p.passengerTypeCode} - {key}</b>
                </div>
                {status === 'success' ? <CheckCircle2 color="#27ae60" size={24} /> : <AlertCircle color="#ccc" size={24} />}
              </div>
      ...
      <div style={{ textAlign: 'center', marginTop: '3rem' }}>
         <button className="primary" onClick={onAddAll} disabled={Object.values(passengerStatus).some(s => s.status === 'success')} style={{ padding: '1.2rem 4rem' }}>
            AGREGAR TODOS LOS PASAJEROS
         </button>
      </div>
      ...
      {Object.keys(passengerStatus).length === Object.keys(sellResults.passengers).length && Object.values(passengerStatus).every(s => s.status === 'success') && (
         <div style={{ marginTop: '5rem', padding: '4rem', border: '8px solid var(--jetsmart-yellow)', textAlign: 'center', borderRadius: '32px', background: 'rgba(255,243,205,0.1)' }}>
            ...
            <button className="primary" onClick={onCommit} disabled={committing} style={{ padding: '1.5rem 6rem', fontSize: '1.8rem', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
